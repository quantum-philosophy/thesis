...
\end{align*}
It can be seen in \eref{interpolant-smolyak} that a Smolyak interpolant can be
...
It can be seen in \eref{interpolant-smolyak-grid} that it is beneficial to the
...
\]
derived from \eref{interpolant-smolyak-grid} decreases. In words, the values of
...
The final step is to rewrite \eref{interpolant-smolyak} in its hierarchical
...
substituting \eref{interpolant-tensor-delta} into \eref{interpolant-smolyak}.
The result is
\begin{align*}
  \interpolant{n}{\ls}(\g)
...
\eref{interpolant-smolyak-surplus}.
