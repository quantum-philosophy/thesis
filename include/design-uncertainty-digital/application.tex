...
application model given in \sref{thermal-cycling-problem} except for its
periodicity. Each task has a start and a finish time. For task $i =
\range{1}{\nt}$, denote these two time moments by $b_i$ and $d_i$, respectively.
Let also $\v{b} = (b_i)_{i = 1}^\nt$ and $\v{d} = (d_i)_{i = 1}^\nt$. Then other
timing characteristics of the application in question can be derived from
$(\v{b}, \v{d})$. For example, the end-to-end delay---which is the difference
...
\sref{interpolant-problem}. Then the tuple $(\v{b}, \v{d})$ and, hence, the
end-to-end delay in \eref{interpolant-delay} given above depend on \vu. This
delay is a quantity \g that is potentially of interest to the designer. Note
that this \g is nondifferentiable since the $\max$ and $\min$ functions are
such. Hence, \g is nonsmooth, which renders \up{PC} expansions and similar
techniques inadequate for this problem, as illustrated in
...
Let us move on to power. Denote by $\p_{\dynamic, ij}$ the dynamic power
consumed by processing element $i$ when it is executing task $j$. Assume that
this power is constant during the task's execution; note that this simple model
can be improved by representing large tasks as sequences of smaller tasks. Then
the power model given in \eref{power-model} can be rewritten as follows:
\[
  \p_i(t) = \sum_{j = 1}^\nt \p_{\dynamic, ij} \delta_{ij} (t) + \p_{\static, i}(t)
\]
for $i = \range{1}{\np}$ where $\delta_{ij}(t)$ is the indicator function of the
event that processing element $i$ is running task $j$ at time $t$, and
$\p^\static_i(t)$ is the static power consumed by processing element $j$ at time
$t$. The last component depends on time since temperature changes over time, and
power and temperature are interdependent, which is discussed in
\sref{power-model} and \sref{power-temperature-interplay}. The total energy
consumed by the system during an application run can be computed by integrating
the above equation over the time span of the application---which is demarcated
by the minuend and subtrahend in \eref{interpolant-delay}---and this integral
can be estimated using a power profile \mp as follows:
