...
$\transform$ needed in \eref{interpolant-transformation} and depicted in
\fref{interpolant-application} is to be decided upon. In this example, we base
$\transform$ on the one shown in \eref{probability-transformation} and reduce
$\vu: \Omega \to \real^\nu$ to $\vz: \Omega \to [0, 1]^\nz$ so that the latter
is uniformly distributed. In this case, the rightmost $\transform_1$ in
\eref{probability-transformation} is simpler than what it is according to its
...
\eref{probability-transformation} is engaged; it eliminates redundant stochastic
...
\sref{interpolant-transformation}. Namely, the codomain of \vz is $[0, 1]^\nz$,
...
The leftmost box in \fref{interpolant-application} represents a ``black-box''
...
In the graph shown in \fref{interpolant-density}, the blue line shows the
...
\sref{interpolant-transformation}. Third, an interpolant of the resulting
composition $\g \circ \transform$ is constructed; this stage is detailed in
\sref{interpolant-construction}. Fourth, an estimation of the probability
