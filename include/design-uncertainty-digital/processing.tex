...
in accordance with \aref{interpolant-evaluation}. Having collected a large
enough set of samples denoted by $G$, the density of \g can be estimated by
employing such techniques as kernel density estimation \cite{hastie2013}. Having
$G$ at one's disposal, other statistics about \g, such as probabilities of
various events, can be straightforwardly estimated.

In addition, the expectation and variance of \g can be computed without
sampling. Using an adequate $\transform$ discussed in
\sref{interpolant-transformation}, \g can be reparameterized in terms of
independent variables that are uniformly distributed on $[0, 1]^\nz$. This means
that the density function of \vz equals unity. Therefore, using
\eref{interpolant-sparse}, we have the following analytical expression for the
expectation:
\[
...
\rref{interpolant-multidimensional-output}.

\begin{remark} \rlab{interpolant-multidimensional-output}
...
in \eref{interpolant-stop} for different outputs. In that case, one also needs
to revisit \eref{interpolant-score} and devise a more sensible strategy for
