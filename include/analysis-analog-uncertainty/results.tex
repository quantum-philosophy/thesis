...
\up{GNU}/Linux machine with Intel Core i7 2.66~\up{GH}z and 8~\up{GB} of
\up{RAM}. All the configuration files used in the experiments are available
...
covariance function in \eref{inference-covariance}, the weighting parameter $w$
and the two length-scale parameters $\ell_\mathrm{SE}$ and $\ell_\mathrm{OU}$
should be set according to the correlation patterns typical for the
manufacturing process at hand \cite{chandrakasan2000, cheng2011}; we set $w$ to
0.7 and $\ell_\mathrm{SE}$ and $\ell_\mathrm{OU}$ to half the radius of the
wafer. The threshold parameter $\eta$ of the model-order-reduction procedure
described in \sref{linear-algebra} and utilized in \eref{inference-reduction}
should be set high enough to preserve a sufficiently large portion of the
variance of the data and, thus, to keep the modeling sufficiently accurate; we
set it to 0.99. The resulting dimensionality \nz of \vz in
\eref{inference-reduction} is found to be 27--28. The parameters $\mu_0$ and
$\tau_\g$ of the prior in \eref{inference-prior} are specific to the considered
technological process; we set $\mu_0$ to 17.5~nm and $\tau_\g$ to 2.25~nm. The
parameters $\sigma_0$ and $\nu_\g$ in \eref{inference-prior} determine the
...
\eref{inference-prior} represents the precision of the equipment utilized for
...
in \eref{inference-prior} has the same interpretation as $\nu_\g$; we set it to
10 as well. In \eref{inference-proposal}, $\nu$ and $\alpha$ are tuning
parameters, which can be configured based on experiments; we set $\nu$ to 8 and
$\alpha$ to 0.5. The number of samples is another tuning parameter, which we set
to $10^4$; the first half of these samples is ascribed to the burn-in period,
leaving $5 \times 10^3$ effective samples. For the optimization in
...
interest are shown in \fref{inference-example} where the \acf{NRMSE} is below
...
post-processing stage, Stage~4 in \fref{inference-overview}, is not given as it
...
\tref{inference-points}. The computation time grows along with \np. This
...
contributes four thermal nodes to the thermal \up{RC} circuit that temperature
analysis is based on; recall \sref{temperature-model}. Therefore, temperature
...
\hiddensubsection{Deviation of the Measurement Noise}

\inputtable{inference-noise}
In this subsection, we vary the standard deviation of the measurement noise,
which corrupts $H$. The considered cases are 0, 0.5, 1, and \celsius{2}
