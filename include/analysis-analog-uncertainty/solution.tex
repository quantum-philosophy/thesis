...
\sref{bayesian-statistics}, in order to derive the distribution of \g. The
...
instance, in the example given in \sref{inference-example}, an observation at a
site is a temperature profile $\mq \in \real^{\np \times \ns}$, which is an
matrix capturing temperature of \np processors at \ns time moments as defined in
\eref{temperature-profile}. Denote by $H = \{ \hh_i \}_{i = 1}^{\hnd}$ the
...
threshold; recall \sref{inference-example}. It can be seen in
...
\sref{power-temperature-interplay}. The second transition is undertaken by
...
\begin{equation} \elab{inference-covariance}
  k(r_1, r_2) = \sigma_\g^2 \left(w \, k_\mathrm{SE}(r_1, r_2) + (1 - w) k_\mathrm{OU}(r_1, r_2)\right)
\end{equation}
for $r_1 \in \real^2$ and $r_2 \in \real^2$ where
\begin{align*}
  k_\mathrm{SE}(r_1, r_2) & = \exp\left(-\frac{\norm[2]{r_1 - r_2}^2}{\ell_\mathrm{SE}^2}\right) \text{ and} \\
  k_\mathrm{OU}(r_1, r_2) & = \exp\left(-\frac{\absolute{\,\norm[2]{r_1} - \norm[2]{r_2}\,}}{\ell_\mathrm{OU}}\right)
\end{align*}
...
[0, 1]$ is a weighting coefficient; $\ell_\mathrm{SE} > 0$ and $\ell_\mathrm{OU}
...
\cite{chandrakasan2000, cheng2011}. More specifically, $k_\mathrm{SE}$ imposes
similarities between locations on the wafer that are close to each other, and
$k_\mathrm{OU}$ imposes similarities between locations that are at the same
distance from the center of the wafer. The parameters $\ell_\mathrm{SE}$ and
$\ell_\mathrm{OU}$ control the extend of these similarities, that is, the range
...
$\ell_\mathrm{SE}$, and $\ell_\mathrm{OU}$---are assumed to be determined prior
to our analysis based on the knowledge of the correlation patterns typical for
the production process utilized; see \cite{marzouk2009} and the references
therein.
...
In addition, the parameters $\u_\g$ in \eref{inference-prior} are now known, and
...
priors in \eref{inference-prior}, we obtain $p(\u)$ in \eref{bayes-theorem}.
...
\eref{inference-likelihood}, and the prior $p(\u)$, which is the product of the
...
value $\u^*$ of \u that maximizes \eref{inference-posterior} and, hence, has the
...
posterior distribution of \u in \eref{inference-posterior}. Specifically, in
