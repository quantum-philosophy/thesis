...
uncertainty on a design, and---as it is also the case with the framework
presented in \cref{design-uncertainty-analog}---that is straightforward to use
...
framework is streamlined because it has the same low entrance requirements as
sampling techniques: one only has to be able to evaluate the quantity given a
set of deterministic parameters. Moreover, the framework can be utilized in
scenarios with limited knowledge of the joint probability distribution of the
uncertain parameters, which are common in practice.
