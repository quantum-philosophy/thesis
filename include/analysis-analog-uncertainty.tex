...
\eref{temperature-profile}. Denote by $H = \{ \hh_i \}_{i = 1}^{\ndm}$ the
...
posterior distribution of \t in \eref{inference-posterior}. Specifically, in
...
\sref{linear-algebra} and utilized in \eref{inference-reduction} should be set
...
resulting dimensionality \nz of \vz in \eref{inference-reduction} is found to be
27--28. The parameters $\mu_0$ and $\tau_\g$ of the prior in
\eref{inference-prior} are specific to the considered technological process; we
set $\mu_0$ to 17.5~nm and $\tau_\g$ to 2.25~nm. The parameters $\sigma_0$ and
$\nu_\g$ in \eref{inference-prior} determine the precision of the information
...
The parameter $\tau_\epsilon$ in \eref{inference-prior} represents the precision
...
optimization in \sref{inference-optimization}, we use the quasi-Newton algorithm
