...
Component~3 is to define a likelihood function, which is where the observed
information is taken into account; see \sref{inference-preliminaries}. In our
case, the observed information is the measurements $H$ stacked into
$\vh^\measured$ as described in \sref{data-model}. Since the measurement process
is not perfect, we should also take into consideration the measurement noise. To
this end, for a given \g, the observed $\vh^\measured$ is assumed to deviate
from the data model prediction \vh as follows:
