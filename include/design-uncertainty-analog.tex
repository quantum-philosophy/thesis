...
The technique used to solve the thermal system in \eref{thermal-model} belongs
...
Multiplying both sides of \eref{thermal-model-inner} by $e^{- \m{A} t}$ and
noting that
\[
  e^{-\m{A} t} \frac{\d{\vs(t)}}{\d{t}} = \frac{\d{e^{-\m{A} t} \vs(t)}}{\d{t}} + e^{-\m{A} t} \m{A} \vs(t),
\]
we obtain the exact solution of \eref{thermal-model-inner} over a time interval
...
$\correlation{\vh} = \m{V} \m{\Lambda} \m{V}^T$ (see \sref{thermal-model} for
...
in \sref{preprocessing}. This reduction is based on the eigendecomposition
undertaken in \sref{probability-transformation}. The intuition is that, due to
