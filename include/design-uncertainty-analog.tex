...
$10^5$ in \tref{chaos-accuracy-0-5}. However, the aforementioned main trend is
still clear.
...
\tref{chaos-accuracy-0-5} is bounded by 1.83\%; see $\epsilon_f$ for $\lc \geq
3$ and $\no \geq 10^4$. In order to give a better intuition about the proximity
of the two methods, \fref{chaos-example-density} displays the \acp{PDF}
...
\tref{chaos-accuracy-1}: $10^3$ samples leave an error of more than 7\% for the
...
First, we vary the number of processing elements \np, which directly affects
the dimensionality of the uncertain parameters $\vu: \Omega \to \real^\nu$;
recall \sref{chaos-application}. As before, we report the results obtained for
various weight coefficients $w$, which impacts the number of the independent
variables $\vz: \Omega \to \real^\nz$ preserved after the model order reduction
procedure described in \sref{probability-transformation}. The results,
including the dimensionality \nz of \vz, are given in
\tref{speed-processing-elements} where the considered values for \np are $\{
2^n \}_{n = 1}^5$, and the number of time steps \ns is set to $10^3$. It can be
seen that the correlation patters inherent to the fabrication process
