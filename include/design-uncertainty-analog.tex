...
\sref{chaos-example} and \sref{chaos-results}. The result is depicted in
\fref{chaos-example} by a blue line, which corresponds to the temperature of one
...
\fref{chaos-example}; the maximum temperature is approaching \celsius{100}.
...
\fref{chaos-example-density} described in \sref{chaos-results}.
...
is given in \eref{chaos-length-total-order}, which behaves as $\nz^\nc / \nc!$
...
\sref{chaos-results}. In addition, the \ac{PDF} of the temperature at that time
...
Another observation in \tref{chaos-scaling-elements} is that the slope of the
execution time of the \ac{MC} technique is low, which illustrates the well-known
fact that the workload per \ac{MC} sample is independent of the number of
stochastic dimensions. On the other hand, the rows with $\nz > 10$ hint at the
curse of dimensionality, which \ac{PC} expansions suffer from. However, even in
high dimensions, the proposed framework significantly outperforms the approach
based on \ac{MC} sampling. For instance, in order to analyze a power profile
with 10\textsuperscript{3} steps of a platform with 32 cores, the \ac{MC}
...
number of steps \ns in power and temperature profiles. The results for a
quad-core platform are reported in \tref{chaos-scaling-steps}. Due to the long
execution times demonstrated by the \ac{MC} approach, its statistics for high
values of \ns are extrapolated based on a smaller number of samples, that is,
$\no < 10^4$. As it is noted with respect to \tref{chaos-scaling-elements}, we
observe the dependency of the \ac{PC} expansions on the dimensionality \nz of
\vz, which is two for $w = 0$ and five for the other two values of $w$; see
...
linearly with \ns, which is expected. However, the proposed framework shows a
vastly superior performance being up to five orders of magnitude faster than
the \ac{MC} alternative.
...
second reason is the particular solution process used in the framework to solve
the thermal model and construct \ac{PC} expansions in a stepwise manner shown in
\eref{chaos-recurrence}.
