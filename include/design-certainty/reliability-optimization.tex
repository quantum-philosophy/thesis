...
\sref{dynamic-steady-state-solution} for mitigating the damage caused by
...
experimental results are reported in \sref{thermal-cycling-result}.
...
the list scheduling policy \cite{adam1974}. The input to the scheduler is a set
of priorities of the tasks and a mapping of the tasks onto the processing
elements; other parameters are implicitly known to the scheduler. The schedule
...
to find a schedule \schedule that maximizes
...
\begin{align} \elab{thermal-cycling-constraints}
  \begin{split}
    & \max_{i = 1}^\nt t_{\ns_i}(\schedule) \leq \period \text{ and} \\
    & \norm[\infty]{\mq(\schedule)} \leq \q_\maximum
  \end{split}
\end{align}
where $\mean_i$ is the \ac{MTTF} of processing element $i$ computed according to
\eref{thermal-cycling-mean-time}; $t_{\ns_i}$ is the completion time of task
$i$; \mq is the dynamic steady-state temperature profile of the system; and
$\norm[\infty]{\cdot}$ denotes the uniform norm. The first constraint enforces a
deadline on the schedule, and the second constraint enforces a maximum
temperature $\q_\maximum$ on the resulting \mq.
