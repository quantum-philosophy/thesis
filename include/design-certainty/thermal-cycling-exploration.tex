...
In this context, a heterogeneous platform with \np processing elements is to
...
Each pair of a task and a processing element is characterized by an execution
time and a power consumption. Lastly, the application is assumed to be scheduled
using a list scheduler whose inputs are priorities of the tasks and a mapping of
the tasks onto the processing elements.
...
Recall that we have a heterogeneous platform with \np processing elements and a
...
system under consideration. The time to failure of each processing element is
modeled using the reliability model presented in \sref{reliability-model}. With
a slight abuse of notation, let $\mean_i$ be the \ac{MTTF} of processing element
$i$ computed according to \eref{reliability-mean-time}. Our optimization
objective is to find a schedule that maximizes
