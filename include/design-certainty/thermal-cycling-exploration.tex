...
In this context, the \np processors correspond to processing units, and they are
to execute a periodic application with \nt tasks. The application is given as a
directed acyclic graph whose vertices and edges correspond to the tasks and to
data dependencies between the tasks, respectively; see
\fref{thermal-cycling-task-graph} for an example. The period of the application
is denoted by \period and assumed to be equal to the application's deadline.
Each pair of a task and a processor is characterized by an execution time and a
power consumption. Lastly, the application is assumed to be scheduled using a
...
Lastly, the experimental results are reported in \sref{thermal-cycling-results}.
...
Recall that we have a platform with \np processors (processing units) and a
periodic application with \nt tasks and a period \period, and that, in this
context, \mq stands for the dynamic steady-state temperature profile of the
system under consideration. The time to failure of each processor is modeled
using the reliability model presented in \sref{reliability-model}. With a slight
abuse of notation, let $\mean_i$ be the \ac{MTTF} of processor $i$ computed
according to \eref{reliability-mean-time}. Our optimization objective is to find
...
the processors are fed to the list scheduler. The scheduler produces a schedule.
If the schedule violates the deadline constraint given in
...
parameters of the processors and tasks, a power profile \mp is constructed, and
the corresponding temperature profile \mq is computed using our technique
presented in \sref{dynamic-steady-state-solution}. If the profile violates the
temperature constraint in \eref{thermal-cycling-constraints}, the chromosome is
penalized proportionally to the amount of violation and is not further
processed. Otherwise, the \ac{MTTF} of each processor is estimated according to
