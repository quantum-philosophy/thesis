...
motivational example in \sref{thermal-cycling-example}. The utilized system and
reliability models are then presented in \sref{thermal-cycling-system-model} and
\sref{reliability-model}, respectively. In \sref{thermal-cycling-problem}, we
...
side of \fref{thermal-cycling-example}. According to the reliability model that
we shall describe shortly, these two changes improve the lifetime of the system
by around 45\% and 55\%, respectively, relative to the initial schedule.
...
their characteristics. In order to obtain this information, dynamic steady-state
temperature analysis has to be undertaken.

\subsection{System Model}
\slab{thermal-cycling-system-model}

The system model is based on the one in \sref{system-model}. In this case, the
\np processors represent \np \up{CPU}s that are to execute a periodic
...
equal to the deadline. Each pair of a task and a processor is characterized by
an execution time and a power consumption. Lastly, the application is assumed to
be scheduled using a list scheduler whose inputs are priorities of the tasks and
a mapping of the tasks onto the processors.
...
with \nt tasks and a period $\period$ (see \sref{thermal-cycling-system-model}),
and that, in this context, \mq stands for the dynamic steady-state temperature
profile of the system under consideration. The time to failure of each processor
is modeled using the reliability model presented in \sref{reliability-model}.
With a slight abuse of notation, let $\mean_i$ be the \ac{MTTF} of processor $i$
computed according to \eref{reliability-mean-time}. Our optimization objective
is to find a schedule that maximizes
...
the processors are fed to the scheduler; see
\sref{thermal-cycling-system-model}. The scheduler produces a schedule. If the
schedule violates the deadline constraint given in
...
parameters of the processors and tasks discussed in
\sref{thermal-cycling-system-model}, a power profile \mp is constructed, and the
corresponding temperature profile \mq is computed using our technique presented
in \sref{dynamic-steady-state-solution}. If the profile violates the temperature
constraint in \eref{thermal-cycling-constraints}, the chromosome is penalized
proportionally to the amount of violation and is not further processed.
Otherwise, the \ac{MTTF} of each processor is estimated according to
\eref{reliability-mean-time}, and the fitness function in
