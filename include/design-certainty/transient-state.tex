...
Suppose for now that the power consumption of the processing elements does not
change over time; hence, $\vp(t) = \vp$. Then
\eref{temperature-differential-original} is a system of linear ordinary
...
\[
  \tm{A} = -\m{C}^{-1} \m{G}.
\]
The solution belongs to the family of so-called exponential integrators, which
have good stability properties; refer to \cite{hochbruck2010} for an overview.
