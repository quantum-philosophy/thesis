...
\[
  \tm{A} = -\m{C}^{-1} \m{G}.
\]
We apply a solution technique from the family of exponential integrators, which
have good stability properties; refer to \cite{hochbruck2010} for an overview.
Multiplying both sides of the above system by $e^{- \tm{A} t}$ and noting that
\[
  e^{-\tm{A} t} \frac{\d \tvs(t)}{\d t} = \frac{\d e^{-\tm{A} t} \tvs(t)}{\d t} + e^{-\tm{A} t} \tm{A} \tvs(t),
\]
we obtain
\[
  \tvs(t) = e^{\tm{A} t} \tvs(0) +  e^{\tm{A} t} \int_0^t e^{-\tm{A} s} \m{C}^{-1} \tm{B} \vp(s) \d s,
\]
