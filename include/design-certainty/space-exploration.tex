...
side of \fref{thermal-cycling-example} where the height of a task represents its
...
\fref{thermal-cycling-example}. If we also swap T2 and T4, the number of cycles
of P1 drops to one, which is depicted on the right-hand side of
\fref{thermal-cycling-example}. According to the reliability model that we shall
...
\ac{MTTF} of processor $i$ computed according to \eref{reliability-mean-time}.
...
using our technique presented in \sref{dynamic-steady-state-solution}. If the
...
is based on the linear approximation discussed in \sref{interdependence}. The
