...
requirements. Let also $F(\cdot | \vg)$ be the distribution function of \life,
which gives the probability of failure before a certain time moment, where \vg
is a vector of parameters. The expectation $\expectation{\life}$ is called the
\ac{MTTF}. Lastly, the complementary distribution function of \life is
\[
  R(t | \vg) = 1 - F(t | \vg),
\]
...
stress that the corresponding processing element is exposed to. Each $\life_i$
is also assigned an individual reliability function $R_i(\cdot | \vg)$ modeling
the failures due to that stress. The structure of $R(\cdot | \vg)$ with respect
to $\{ R_i(\cdot | \vg) \}_{i = 1}^\np$ is problem specific, and it can be
...
system fail, and that $\{ \life_i \}_{i = 1}^\np$ are conditionally independent
...
wear change over the period. Then the distribution of $\life_i$ should reflect
this aspect, which is not yet prominent in \eref{weibull-distribution}. In order
to take it into account, the period is split into \nk{i} time intervals $\{
