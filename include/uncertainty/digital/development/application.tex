...
strategic invocations of the simulator, it delivers a light surrogate
$\interpolant{\nz}{\ls}(\g)$ for the simulator; in \fref{frame-application}, it
corresponds to the small rounded rectangle.
...
this case, the interpolant $\interpolant{\nz}{\ls}(\g)$ should be sampled, which
is represented by the rightmost box in \fref{frame-application}. The operation
corresponds roughly to \aref{frame-evaluation}: the interpolant takes $\z_1$ and
$\z_2$ and returns an approximation of \g at that point. Recall that the
computational cost of sampling is negligible since \g is not involved. The
collected samples, which are denoted by $G$ in \fref{frame-application}, are
then used in order to compute an estimate of the distribution of \g.

\inputfigure{frame-density}
\fref{frame-density} depicts the result. The blue line shows the \ac{PDF} of \g
computed by applying kernel density estimation \cite{hastie2013} to the obtained
set of samples $G$. The yellow line---which is barely visible behind the blue
...
one. Additionally, the orange line illustrates the estimate that one would
obtain if one sampled \g directly using the same number of \g's evaluations as
the one consumed by our framework. It can be seen that, given the same budget,
the solution delivered by our technique is substantially closer to the true one
than the one delivered by naïve sampling.
