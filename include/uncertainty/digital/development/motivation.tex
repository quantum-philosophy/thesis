Due to its nature, the variability coming from process variation is typically
smooth and well behaved. In such cases, uncertainty quantification based on
\ac{PC} expansions \cite{xiu2010} and other approximation techniques making use
...
\fref{frame-motivation} shows a \ac{PC} expansion of level nine ($\lc = \lq =
...
problem. For the purpose of the experiment, our technique is constrained to make
use of the same number of points as the \ac{PC} expansion does. The result is
the blue curve in \fref{frame-motivation}, and the adaptively chosen points are
plotted on the horizontal axis. It can be seen that the approximation is good,
and, in fact, it would be indistinguishable from the true response with a few
additional points. One can note that the adaptive procedure started to
...
\cite{hastie2013}. The crucial point to note is that, analogous to the \ac{PC}
...
operation costs practically nothing in terms of the computation time.
