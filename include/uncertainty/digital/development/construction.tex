...
The integration grid should be fully nested in order to gain additional
computational efficiency, which is explained in \xref{sparse-interpolation}.
Such a grid can be constructed using the family of Newton--Cotes rules
\cite{ma2009}. In one dimension, a Newton--Cotes rule is merely a set of
...
while the latter does not. Now, in \xref{sparse-interpolation}, we postulate
...
right-hand side of \eref{frame-sparse}.
...
to the rules in \sref{frame-adaptivity}. Now we discuss the implementation.
...
\aref{frame-evaluation}. This subroutine is also involved in
\aref{frame-construction}; see line~5. Let us make a couple of observations
