...
these features are interconnected and can be summarized as follows: \one~the
ability to efficiently tackle multidimensional problems, \two~the ability to
perform gradual refinement of the approximation with a natural error control,
and \three~the ability to make the refinement fine-grained and, therefore, gain
further efficiency.
...
presented in \xref{sparse-interpolation} as well as here is based on the
...
limiting in practice. As shown in \xref{sparse-interpolation}, \g can be
...
Due to the reason that is to be made clear in \sref{frame-adaptivity}, the
interpretation of \eref{frame-sparse} used in this section is different from the
interpretation used in \xref{sparse-interpolation}. Specifically, $\ls \in
...
of their full-fledged counterparts defined in \xref{sparse-interpolation}.
...
chosen rules have to satisfy a certain condition, which is motivated in
...
adaptivity presented in \cite{ma2009}. The open ones, on the other hand, do not
...
\xref{sparse-interpolation} and, more specifically, in
\eref{interpolant-tensor-grid} together with \eref{quadrature-tensor-index}.
...
following piecewise linear functions. For $i = 0$ and $j = 0$,
\[
  e_{00}(\x) = 1.
\]
...
interpolation are depicted in \fref{frame-basis}. In multiple dimensions, the
basis functions are formed as shown in \eref{interpolant-tensor-basis} and
\eref{quadrature-tensor-index}.

Lastly, let us calculate the volumes (the integrals over the whole domain) of
the basis functions, which we denote by $w_{ij}$ and make use of in the
continuation. For $i = 0$ and $j = 0$,
\[
  w_{00} = 1.
\]
...
much fewer collocation nodes than the other one; recall \fref{frame-motivation}.
However, if we followed the usual construction procedure described in
\xref{sparse-interpolation}, we would not be able to benefit from this
...
\xref{sparse-interpolation}---see \eref{interpolant-sparse-surplus}---and
$w_{\vi \vj}$ are products of the one-dimensional volumes shown in
\sref{frame-basis}. Then this score is used in order to guide the algorithm as
we explain now.
...
form the increment given on the right-hand side of \eref{frame-sparse}.
...
\eref{frame-sparse} is made as follows. First, each index can be picked at most
...
process in \eref{frame-sparse}. Apart from the natural constraints on the
...
interpolation in multiple dimensions. The main equation is \eref{frame-sparse}
where $\Delta(\g \circ \transform)(\vx_{\vi \vj})$, $\vx_{\vi \vj}$, and $e_{\vi
\vj}$ are the ones in \eref{interpolant-sparse-surplus}, \sref{frame-grid}, and
\sref{frame-basis}, respectively, and the interpolation is undertaken according
to the rules in \sref{frame-adaptivity}. Next, we discuss the implementation.
...
\eref{frame-sparse} into one. This requires storing a level index $\vi = (i_k)
...
\eref{frame-sparse}. The progress is captured by a structure \texttt{state}. The
\texttt{strategy} object represents the adaptation strategy utilized, and it
operates in accordance with \sref{frame-adaptivity}. The \texttt{Continue?}
...
see \sref{frame-grid} for more details.
...
\eref{frame-sparse}; that is, there is no separation into the individual
...
arbitrary points; see \sref{frame-basis}.
...
is consolidated in \aref{frame-construction} and \aref{frame-evaluation}.
