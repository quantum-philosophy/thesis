...
Due to the reason to be discussed in \sref{frame-adaptivity}, the interpretation
of \eref{frame-sparse} is different from the one used in
...
defined in \xref{sparse-interpolation}.
...
\eref{interpolant-sparse-surplus} and \eref{frame-basis-volume}, respectively.
...
\eref{frame-sparse} is made as follows. First of all, each index can be picked
...
\vj}$ are the ones given in \eref{interpolant-sparse-surplus},
\sref{frame-grid}, and \sref{frame-basis}, respectively, and the interpolation
is undertaken according to the rules given in \sref{frame-adaptivity}. Now we
