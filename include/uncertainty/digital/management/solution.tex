...
Let us discuss training. The model depicted in \sref{network-model} has a large
number of parameters that have to be learned; they are primarily various weights
and biases inside the layers. For this purpose, $G_1$ is utilized. The training
is undertaken via backpropagation through time using stochastic gradient descent
...
Let us now discuss validation. As it is the case with arguably all nontrivial
machine-learning models, the one presented in \sref{network-model} has a number
of hyperparameters. They include the number of cells \nc, number of units per
cell \nu, and probability of dropout $\rho_\drop$, which are introduced in
\sref{recurrent}. Unlike ordinary parameters, which are to be optimized during
training (see \sref{training}), the hyperparameters are to be set prior to
training and kept unchanged thereafter. The impact of the hyperparameters is
profound; therefore, they should be carefully selected.

The validation set $G_2$ is used to assess the performance of the model trained
(using $G_1$ as usual) under different configurations of the hyperparameters of
the model. As before, the error metric utilized is the \ac{MSE}, and it is
beneficial to perform these computations in batches. The trained model that has
the best performance on $G_2$ is then chosen as the one to be used.
