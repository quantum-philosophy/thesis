...
= 1$ in \eref{brain-profile}. In this regard, the data set provides two apposite
...
recurrent layers \nl (the blue boxes in \fref{brain-model}), number of units per
layer \nu (the double circles in \fref{brain-model}), and probability of dropout
$\rho_\drop$; these hyperparameters are introduced in \sref{brain-model}.
Specifically, we let \nl be in $\{ 1, 2, 3, 4, 5 \}$, \nu be in $\{ 100, 200,
...
\sref{brain-learning}. The maximum budget granted to one configuration is set to
...
\sref{brain-data}, individual learning sessions do not have any overhead in this
regard. Note also that the machine utilized in these experiments has no modern
\up{GPU}s; therefore, there is an immense room for performance improvement.
