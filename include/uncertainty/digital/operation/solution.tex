...
on the left-hand side of \fref{brain-predictive-model}. Similarly, the output is
a single \ng-dimensional data point, which is depicted on the right-hand side of
\fref{brain-predictive-model}. The input $\vg_{ij}$ is the value of the resource
...
\fref{brain-predictive-model}, they are represented by a group of blue boxes.
...
by an orange box in \fref{brain-predictive-model}. Unlike the recurrent layers,
...
\sref{brain-data-pipeline} is the data set $G$, which is split into three parts:
...
pipeline, which are also displayed at the bottom of \fref{brain-overview}.

Let us discuss training first. The model depicted in
\sref{brain-predictive-model} has a large number of parameters that have to be
...
\fref{brain-feed}, in which the schematic representation in
\fref{brain-predictive-model} has been simplified even further and rotated
...
memory. It can also be seen in \fref{brain-feed} how information flows from one
time step to the next, which implicitly gives the model access to the whole
...
\sref{brain-predictive-model} has a number of hyperparameters. Examples include
