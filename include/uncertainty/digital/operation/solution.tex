...
\fref{brain-overview} and is explained in \sref{brain-data}. Next, the resulting
profiles are utilized in order to obtain an adequately trained predictive model.
The modeling part is covered in \sref{brain-model} while the learning part is
explained in \sref{brain-learning}; the latter can also be seen at the bottom of
\fref{brain-overview}. The above operations are to be undertaken offline while
...
\fref{brain-model}, they are represented by a group of blue boxes. The network
can be made as many layers deep as needed. Each layer is composed of a number of
units, which are depicted as double circles in \fref{brain-model}. The number of
...
is depicted by an orange box in \fref{brain-model}. Unlike the recurrent layers,
...
of \ac{LSTM} layers and a linear regression layer. Due to its internal memory,
...
\fref{brain-overview}. Similar to other nontrivial models, the one presented in
\sref{brain-model} has a number of hyperparameters. Examples include the number
