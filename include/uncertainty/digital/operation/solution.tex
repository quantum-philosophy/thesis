...
Our workflow is illustrated in \fref{brain-overview}. First, we note that the
...
in \eref{brain-dataset} from given raw data. This processing part can be seen at
the top of \fref{brain-overview} and is explained in \sref{brain-data}. Next,
the resulting profiles are used in order to obtain an adequately trained
predictive model. The modeling part is covered in \sref{brain-model} while the
learning part is explained in \sref{brain-learning}; the latter can also be seen
at the bottom of \fref{brain-overview}. The above operations are to be
...
follows. Consider now the three topmost boxes in \fref{brain-overview}.
...
needs of a particular training session (to be discussed in \sref{brain-results})
...
$G_2$, and $G_3$, respectively; see also \fref{brain-overview}.
...
As emphasized in \sref{brain-introduction} and \sref{brain-problem}, our goal is
...
\inputfigure{brain-model}
A schematic representation of our model can be seen in \fref{brain-model}. Note
...
on the left-hand side of \fref{brain-model}. Similarly, the output is a single
\ng-dimensional data point, which is depicted on the right-hand side of
\fref{brain-model}. The input $\vg_{ij}$ is the value of the resource usage of
...
called cells. They are shown as a group of blue boxes in \fref{brain-model}. The
network can be made as many layers deep as needed. Each cell is composed of a
number of units, which are depicted as double circles in \fref{brain-model}. The
...
by an orange box in \fref{brain-model}. Unlike the recurrent layers, which
...
Recall that the output of the data pipeline described in \sref{brain-data} is
...
of \fref{brain-overview}.

Let us discuss training first. The model depicted in \sref{brain-model} has a
...
profile---which can be seen in \eref{brain-profile}---has multiple data points
...
\fref{brain-feed}, in which the schematic representation in \fref{brain-model}
...
\fref{brain-feed} how information flows from one time step to the next, which
...
to be zero. For instance, $\hat{\vg}_{i, \nsi{i} + 1}$ in \fref{brain-feed} has
no $\vg_{i, \nsi{i} + 1}$ to be compared with, and this $\vg_{i, \nsi{i} + 1}$
is assumed to be zero.

Let us now discuss validation, which corresponds to the middle blue box in
\fref{brain-overview}. As it is the case with arguably any nontrivial model, the
one presented in \sref{brain-model} has a number of hyperparameters. Examples
...
\sref{brain-results}, we use the latter.
...
helpful to consider the example given in \fref{brain-application}. The process
