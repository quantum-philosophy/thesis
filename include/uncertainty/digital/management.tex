...
$\range{\hat{\vg}_{i,k + 1}}{\hat{\vg}_{i,k + h}}$; in \fref{network-example},
...
\eref{network-trace}. Such a data set can be straightforwardly collected
...
In order to attain the objective established in \sref{network-problem}, we
reside to learning from historical data.

We apply machine learning to the available data in order to construct an
adequate model offline, and this model is then used in order to make predictions
at runtime, assisting the resource manager in accounting for workload
uncertainty.

First, we develop a data-processing pipeline for working with large data sets
that makes the data readily accessible for machine learning. The pipeline is
applied to a data set of real resource-usage traces collected in a large
computer cluster. Second, we construct a model for making fine-grained
long-range prediction of the system's resource usage that relies on the
state-of-the-art recurrent neural networks \cite{goodfellow2016}.
...
In addition, we are the first to address this prediction at the level of
individual tasks executed in the cluster and multiple steps ahead, providing the
resource manager with more detailed and foresighted information and allowing for
more elaborate management of the cluster.
