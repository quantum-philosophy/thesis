...
Although \eref{bayes-correlation} captures certain features that are inherent to
the fabrication process, it is still an idealization. In practice, it can be
...
as those discussed in \sref{chaos-past}. A correlation matrix, on the other
hand, can be readily estimated from measurements and thus is a more probable
input to probabilistic analysis. Hence, we use \eref{bayes-correlation} for the
sole purpose of constructing a correlation matrix of $\set{\u_{\local, i}}_{i =
1}^\np$. For convenience, this correlation matrix is extended by one dimension
in order to accommodate $\u_\overall$ along with $\set{\u_{\local, i}}_{i =
1}^\np$. Thus, the matrix acquires one additional nonzero diagonal element equal
to unity; the resulting matrix is the correlation matrix of \vu denoted by
$\correlation{\vu}$.
...
\sref{chaos-construction}. We let \vz have beta distributions, making \vu and
...
Consider, for example, \eref{chaos-expansion-example}. It can be seen that the
