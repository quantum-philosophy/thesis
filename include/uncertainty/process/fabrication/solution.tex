...
processed via Bayesian inference, as introduced in \xref{bayesian-statistics},
...
\xref{bayesian-statistics}, and they are to be discussed further in what
...
\sref{bayes-motivation}. It can be seen in \fref{bayes-overview} that Stage~2
...
\sref{power-model}. The first transition is due to the dependence of the leakage
current on the effective channel length, which is implicitly present in
\eref{static-power}. The transition can then be made by means of the model in
\eref{static-power} or any of its variations; see, for instance,
...
of the power-temperature interdependence described in \sref{power-model}. The
...
significant. Although all of the above parameters of \g can be inferred from
data, for simplicity, we focus only on $\mu_\g$ and $\sigma_\g^2$. The rest of
the parameters---namely $w$, $\ell_\SE$, and $\ell_\OU$---are assumed to be
...
assume that the designer is interested in all of the sites, which results in
$\nd \np$ locations in total. Let $\vg \in \real^{\nd \np}$ store the values of
\g at these locations. Second, the dimensionality of the problem is reduced
further via the \ac{KL} decomposition, which is introduced in
...
\vg computed via \eref{bayes-covariance}. The result is
...
contains all of the necessary information about the quantity of interest \g.
