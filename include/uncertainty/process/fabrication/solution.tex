...
model of static power can be constructed via a fitting procedure applied to a
data set of \up{SPICE} simulations of reference electrical circuits. The only
...
second transition is made by combining the static component of power with the
dynamic power that corresponds to the workload being considered. The resulting
total power and the relevant temperature-related information---such as the
floorplan and thermal parameters of the platform---are fed to a temperature
simulator in order to acquire the corresponding temperature \h, which is
discussed in detail in \cref{certainty-development}.
...
motivated in \sref{bayes-motivation}, this is especially helpful in
decision-making.
