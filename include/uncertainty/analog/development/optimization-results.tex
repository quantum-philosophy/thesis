...
\up{GNU}/Linux machine equipped with 16 processors, each being an Intel Xeon
E5520 2.27~\up{GH}z, and 24~\up{GB} of \up{RAM}. All the configuration files
...
Regarding the correlation function in \eref{bayes-correlation}, the weight
...
described in \xref{probability-transformation} is set to preserve 95\% of the
variance of the problem. The parameter $\gamma$ in
\eref{chaos-anisotropic-weight} tuning the anisotropy of \ac{PC} expansions and
...
grids where each processing element occupies 2~×~2~\power{mm}{2}. The sampling
...
stopping condition in \aref{chaos-dynamic-steady-solution-iterative} is that the
...
Our first goal is to evaluate the accuracy of the proposed solution. To this
...
the quantity given in \eref{chaos-optimization-quantity}. The operations
performed by the \ac{MC}-based approach for one sample are exactly the same as
those performed by our technique for one quadrature point. The only difference
is that no model order reduction of any kind is undertaken prior to \ac{MC}
sampling, which ensures that the resulting accuracy is not compromised. The
number of \ac{MC} samples is set to \power{10}{4}, which is a practical
assumption based on not only our experience but also the experience from the
literature as discussed in \sref{chaos-transient-results}.
...
\error{E}, \error{Q}, and \error{\life}, respectively. Each error indicator
...
\tref{chaos-optimization-accuracy} additionally contains the number of
...
Our goal now is to assess the speed of the proposed solution. To this end, we
...
in the case of the platform with 32 cores. In addition, one can note that \nz is
the same for the last two platforms. The magnitude of reduction is determined
solely by the assumed correlation structures (see
\sref{chaos-optimization-application}) and the floorplans of the considered
platforms, which we also observe and discuss in \sref{chaos-transient-results}.
...
to \power{10}{4} \ac{MC} samples. Our solution is faster by approximately
...
\eref{chaos-optimization-constraints}. To this end, we employ a genetic
...
fitness, and \eref{chaos-optimization-objective} and
...
applications are randomly generated using \up{TGFF} \cite{dick1998}. In
...
procedure. For instance, $\q_\maximum$ ranges from 90 to \celsius{120},
