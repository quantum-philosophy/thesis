...
\]
for $i = \range{1}{\ns}$ and $\vj \in \sparseindex{\nz}{\lc}$ where the
operation is element-wise, and it can be efficiently undertaken by virtue of the
...
denotes the contribution of the power model given in \eref{chaos-power-model}.
...
is the one shown in \eref{index-total-order-isotropic-length}. This formula
behaves as $\nz^\lc / \lc!$ in the limit with respect to \nz. Regarding \nq, for
quadratures based on the full tensor product given in \eref{quadrature-tensor},
...
\eref{chaos-power-model}---needs to be evaluated and combined with the random
