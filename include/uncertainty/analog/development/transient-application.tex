...
is given in \fref{beta-gaussian} where a beta distribution is fitted to the
...
\eref{inference-correlation}, which is inspired by the correlation patterns
...
input to probabilistic analysis. Therefore, we use \eref{inference-correlation}
...
$\mp_\dynamic$. Similar to \sref{inference-results}, the modeling of the static
part $\f_\static$ is based on \up{SPICE} simulations of a reference electrical
...
extract a vector of independent random variables denoted by \vz. Following the
guidance given in \sref{chaos-transformation}, the most suitable transformation
for the ongoing scenario is the one given in \eref{probability-transformation},
which we denote here by $\transform$; see \eref{chaos-transformation}.

Since the number of stochastic dimensions, which is $\nu = \np + 1$ for \vu,
directly impacts the computational cost of \ac{PC} expansions, which is
elaborated on in \sref{chaos-construction}, one should consider a possibility
for model order reduction before constructing \ac{PC} expansions. To this end,
the model order reduction described in \xref{probability-transformation} is an
...
a suitable polynomial basis as described in \sref{chaos-construction}. We let
...
temperature $\{ \hat{\vq}_{i \vj} \}$. As discussed in \xref{polynomial-chaos},
these computations involve multidimensional integration with respect to the
distribution of \vz, and they should be done numerically using a quadrature;
recall \sref{chaos-construction}. When beta distributions are concerned, the
natural choice is Gauss--Jacobi quadratures, which belong to the class of
Gaussian quadratures; see \xref{sparse-integration}. Then, using
...
such a way that the quadrature is exact for polynomials of total order up to at
least $2 \lc$, that is, twice the level of \ac{PC} expansions, which is
motivated in \sref{chaos-construction}. Consequently, $\lq \geq \lc$ as the
