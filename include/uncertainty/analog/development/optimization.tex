...
The objective in \eref{chaos-optimization-objective} is to minimize the
...
while the other two are probabilistic since they are based on stochastic
quantities. It should be noted that, in the very last constraint, we set an
upper bound on the probability of the expectation of \life; this expectation is
a random variable itself, which is due to the nested structure of the
reliability model discussed in \rref{chaos-nested-uncertainty}.
...
expansions of \eref{chaos-optimization-quantity} inside an optimization
...
description given in \sref{chaos-processing}. Concretely, the two probabilities
needed in \eref{chaos-optimization-constraints} are estimated via sampling the
