...
\sref{probability-theory}. In practice, however, such exhaustive information is
...
\sref{probability-transformation}. It should be understood that, even though the
...
via the \ac{KL} decomposition given in \eref{karhunen-loeve}. In addition, if
...
known. However, as emphasized in \sref{chaos-formulation}, such information is
...
transformation. See \sref{probability-transformation} for more detail.
...
\probability)$; see \sref{probability-theory}. Then, at Stage~3 in
\fref{chaos-overview}, \g is expanded into the following series:
...
described in \sref{sparse-integration}. Specifically, an adequate
...
\eref{quadrature-sparse}. The important aspect to note about
\eref{quadrature-sparse} is the structure of the operator, namely, the index set
$\sparseindex{\nz}{\lq}$, which we discuss in what follows.

The standard choice of $\sparseindex{\nz}{\lc}$ in \eref{chaos-expansion} is the
one in \eref{index-total-order-isotropic}, which is called an isotropic
...
discussed in \sref{sparse-integration}, a quadrature of level \lq is exact for
...
\eref{chaos-expansion} and the one in $\sparseindex{\nz}{\lq}$ in
\eref{chaos-coefficient} should be synchronized.
...
\eref{quadrature-sparse} results in a sparse grid that is exact for the
...
\eref{chaos-expansion} can now be trivially computed as
...
\aref{chaos-construction} where Algorithm~G stands for a subroutine that
...
given in \eref{quadrature-sparse}. The index set used in both
...
projection matrix in \eref{chaos-projection-matrix} is to be calculated only
...
applied to \eref{chaos-expansion}, which is typically followed by kernel density
estimation \cite{hastie2013} or a similar technique. The sampling in this
context can be better understood by rewriting \eref{chaos-expansion}, which is
...
element-wise. In \eref{chaos-coefficients} and \aref{chaos-construction}, \vg
and $\hat{\vg}$ are treated as matrices with \nc rows, and $\g_i$ as a row
vector. The output of Algorithm~G in \aref{chaos-construction} is assumed to be
automatically reshaped into a row vector.
\end{remark}

In what follows, we apply the uncertainty analysis developed in this section to
a number of concrete problems, namely, transient and dynamic steady-state
temperature analysis and reliability analysis and optimization.
