...
\probability)$; see \sref{probability-theory}. Then \g is expanded into the
...
the Smolyak algorithm \cite{smolyak1963} as shown in \eref{quadrature-sparse}.
...
\eref{chaos-expansion} and the index set $\sparseindex{\nz}{\lq}$ in
...
in \eref{chaos-coefficient} needed for calculating the coefficients of those
...
projection matrix in \eref{chaos-projection-matrix} is to be calculated, stored,
and used as shown in \aref{chaos-construction}.
