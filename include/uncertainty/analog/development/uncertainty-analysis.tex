...
\end{equation}
where $\lq \in \natural$ is the quadrature's level, and $\{ \vz_{\vj}: \vj \in
\sparseindex{\nz}{\lq} \} \subset \real^\nz$ and $\{ w_{\vj}: \vj \in
...
\xref{sparse-integration}, a quadrature of level \lq is exact for polynomials of
...
\eref{chaos-expansion} and the content of the index set $\sparseindex{\nz}{\lq}$
in \eref{chaos-coefficient} should be synchronized.
...
\eref{chaos-moments}---should be undertaken element-wise. In
