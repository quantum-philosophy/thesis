...
to \eref{inference-correlation} where the length-scale parameters $\ell_\SE$ and
...
based on simulations of applications randomly generated via \up{TGFF}
...
step of the recurrence in \eref{chaos-recurrence}.
...
\eref{inference-correlation}, that is, the squared-exponential kernel $k_\SE$
...
\error{\variance}, and \error{f}, respectively. The first two are
straightforward to interpret, and they are calculated using the analytical
expressions in \eref{chaos-moments}. The third one is a strong indicator of the
