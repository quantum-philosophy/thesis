...
\sref{prior}. The \ac{MC} approach is set up to preserve the whole variance of
the problem, which means that there is no model order reduction applied, and to
solve the system in \eref{temperature-model-original} directly using the fourth-
...
4 and \power{10}{4}, respectively. The latter value is also consistent with the
...
sampling presented in \cite{diaz-emparanza2002}. Analogous to the previous
subsection, we report the results obtained for different values of the weight
coefficient $w$, which impacts the number of the independent variables $\vz:
\Omega \to \real^\nz$ preserved after applying the model order reduction
