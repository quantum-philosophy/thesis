...
left-hand side of \fref{bayes-motivation}. The gradient from blue to yellow
...
right-hand side of \fref{bayes-motivation}, and the locations of the measured
dies are depicted on the left-hand side of \fref{bayes-application}. It can be
seen that the two maps in \fref{bayes-motivation} closely match each other,
implying that the distribution of \g is reconstructed with a high level of
accuracy.

\inputfigure{bayes-application}
...
the right-hand side of \fref{bayes-application} where $\g_\minimum$ is set to
two standard deviations below the mean value of \g; the probability threshold of
the first action is set to 0.9; the crosses mark both the true and inferred
