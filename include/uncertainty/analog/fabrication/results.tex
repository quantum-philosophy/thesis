...
variation presented in \sref{bayes-solution}. All the experiments are conducted
on a \up{GNU}/Linux machine equipped with Intel Core i7 2.66~\up{GH}z and
...
\eref{bayes-covariance} with a standard deviation of 2.25~nm; \two~perform one
...
the covariance function in \eref{bayes-covariance}, the weight parameter $w$ and
...
in \eref{bayes-reduction} should be set high enough to preserve a sufficiently
...
\vz in \eref{bayes-reduction} is found to be 27 or 28. The parameters $\mu_0$
and $\tau_\g$ of the prior in \eref{bayes-prior} are specific to the considered
technological process; we set $\mu_0$ to 17.5~nm and $\tau_\g$ to 2.25~nm. The
parameters $\sigma_0$ and $\nu_\g$ in \eref{bayes-prior} determine the precision
...
of $\tau_\g$ is based on. The parameter $\tau_\epsilon$ in \eref{bayes-prior}
...
to \celsius{1}. The parameter $\nu_\epsilon$ in \eref{bayes-prior} has the same
...
interest are shown in \fref{bayes-motivation} where the \ac{NRMSE} is below
...
\tref{bayes-sites}. In this and the following tables, we report the optimization
time and sampling time separately, which correspond to Stage~2 and Stage~3 in
\fref{bayes-overview}, respectively. In addition, the sampling time is given for
...
In can be seen in \tref{bayes-sites} that the more data the proposed framework
...
analysis is based on; recall \sref{temperature-model}. Hence, temperature
...
aggregated in \tref{bayes-instances}. It can be seen that the growth of the
...
different, which explains the discordant timing shown in \tref{bayes-points} and
...
The finding can be explained as follows. The temporal distribution of the time
...
\tref{bayes-noise}. It can be observed that the sampling time is approximately
