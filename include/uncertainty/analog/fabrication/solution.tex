...
moments as defined in \eref{temperature-profile}. Denote the collected data by
...
\g$, the proposed technique primarily focuses on the reconstruction of any
...
smaller than a certain threshold; recall the example in \sref{bayes-motivation}.
...
\eref{static-power}. Then the transition can be made by means of the model in
...
\eref{bayes-likelihood}. In order to circumvent this problem, we utilize Markov
chain Monte Carlo sampling and, more specifically, the Metropolis--Hastings
algorithm \cite{gelman2013}, which is outlined in \xref{bayesian-statistics}.
The algorithm operates on an auxiliary distribution called the proposal
