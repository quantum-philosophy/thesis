...
More specifically, instead of \g, we measure an auxiliary parameter \h, which we
...
\two~\h depends on \g, which is signified by $\h = \f(\g)$; and \three~there is
a way to compute \h for a given \g. The last means that \f should be known;
...
\sref{inference-motivation}. Without loss of generality, we adhere to the
following convention. Each die is one potential measurement site, and $\hnd <
\nd$ denotes the number of those sites that are actually measured. Each site
comprises \np measurement points, and there are \ns data instances taken at each
point. For instance, in the example given in \sref{inference-motivation}, an
observation at a site is a temperature profile $\mq \in \real^{\np \times \ns}$,
which is a matrix capturing temperature of \np processing elements at \ns time
moments as defined in \eref{temperature-profile}. Denote by $H = \{ \hh_i \}_{i
...
threshold; recall \sref{inference-motivation}. It can be seen in
...
present in \eref{static-power}. Then the transition can be realized by means of
the model in \eref{static-power} or any of its variations; see, for instance,
...
\sref{power-model}. The second transition is undertaken by combining the static
...
In addition, the parameters $\u_\g$ in \eref{inference-prior-quantity} are now
...
At Stage~4 in \fref{inference-overview}, using the set of samples $G$, the
...
respect to the posterior distribution of \u in \eref{inference-posterior}: in
