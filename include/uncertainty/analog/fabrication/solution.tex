...
processed via Bayesian inference introduced in \sref{bayesian-statistics} in
...
given in \sref{inference-motivation}, an observation at a site is a temperature
profile $\mq \in \real^{\np \times \ns}$, which is a matrix capturing
temperature of \np processing elements at \ns time moments as defined in
\eref{temperature-profile}. Denote by $H$ the collected data and assume that the
...
certain threshold; recall \sref{inference-motivation}. It can be seen in
...
model-order-reduction technique described in \sref{probability-transformation}.
Concretely, we perform the transformation in \eref{model-order-reduction} with
respect to the correlation matrix of \vg computed via
\eref{inference-covariance}. The result is
...
\eref{inference-prior-quantity} are now known, and they are $\u_\g = \{ \vz,
\mu_\g, \sigma^2_\g \}$; see \fref{inference-statistical-model}. The model is
...
can be considered \cite{gelman2013}. Finally, $p(\u)$ in \eref{bayes-theorem} is
...
$\nd \np$-dimensional. Denote by $G$ the corresponding set of samples and let
its cardinality be \no.
