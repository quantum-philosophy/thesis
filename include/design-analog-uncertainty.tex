...
matrix is extended by one dimension in order to tie $u_\overall$ and $\{
u_{\local, i} \}_{i = 1}^\np$ together. In this case, the correlation matrix
obtains one additional nonzero element equal to unity on the diagonal. Taking
into account the variances of the variables, the final covariance matrix
$\covariance{\vu}$ of the whole random vector \vu is formed.

To conclude, the input to our probabilistic analysis is composed of the marginal
distributions of the uncertain parameters \vu, which are beta distributions, and
the corresponding covariance matrix $\covariance{\vu}$.
