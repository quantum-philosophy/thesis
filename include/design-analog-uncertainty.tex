In this chapter, we shift our attention from analyzing to designing with process
...
We proceed as follows. The objective of our study is formulated in
...
\sref{analog-results}. \sref{analog-conclusion} concludes the paper.
...
\vu. The user is required \one~to have prior knowledge or belief about the
probability distribution of \vu and \two~to specify a power model having \vu as
an input. The framework should provide the user with the tools to analyze the
system under a given workload, without imposing any constraints on the nature of
this workload, and obtain the corresponding stochastic power \mp and temperature
\mq profiles with a desired level of accuracy and at low costs.
...
in \sref{power-temperature-interplay}. The others are temperature-related
...
this, we simulate $10^4$ times the example given in \sref{analog-introduction}
...
The major stages of our technique are depicted in \fref{analog-overview}.
