...
\begin{equation} \elab{analog-chaos-moments}
  \begin{split}
...
$i$ is as follows:
\begin{equation} \elab{analog-expansion-example}
  \begin{split}
    \chaos{2}{2}{\vq_i}(\vz)
...
At Stage~5 in \fref{analog-overview}, the constructed expansions are utilized in
...
for example, \eref{analog-expansion-example} that the surrogate model has a
...
\eref{analog-expansion-example}; the same applies to power. Hence, the
...
\eref{analog-expansion-example}.

\inputfigure{analog-example-power}
\inputfigure{analog-example-temperature}
Assume that the dynamic power profile $\mp_\dynamic$ is the one shown in
\fref{analog-example-power}. Having constructed a surrogate with respect to this
...
operation given the formulae in \eref{analog-chaos-moments}. For the whole time
span of $\mp_\dynamic$, these quantities are plotted in
\fref{analog-example-temperature} where the dashed lines correspond to one
...
than a day as we shall see in \sref{analog-results}. In addition, the \ac{PDF}
of the temperature at that time moment can also be estimated. This operation is
undertaken by sampling the surrogate, in which case we might obtain curves
similar to those shown \fref{analog-example-density}, which is a part of a
different example given in \sref{analog-results}.
