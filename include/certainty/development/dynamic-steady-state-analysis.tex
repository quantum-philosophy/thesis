...
discuss further in \sref{reliability-optimization}.
...
solving \eref{temperature-model-original} numerically via the fourth-order
...
\fref{dream-iterative-transient-accuracy}. It can be observed that the number of
...
number of iterations still keeps the computational cost unreasonably high as we
discuss further in \sref{dynamic-steady-state-results}. Moreover, this
...
\cite{huang2009b}. It forgoes transient analysis and resides to static
steady-state analysis instead. To elaborate, it is assumed that, in each time
interval where the system's power consumption is constant, the system
...
steady-state temperature that would be reached if the corresponding power was
...
\fref{dream-static-steady-state-accuracy}. It can be seen that, for example, for an
...
recurrence in \eref{transient-state-recurrence}. The key condition of a dynamic
steady state is
...
techniques. Thus, they are excluded from the discussion.
...
Thermal \up{RC} circuits are considered by means of HotSpot \cite{skadron2003}
...
described in \sref{transient-state-solution} and the one implemented in HotSpot,
...
experiment are depicted in \fref{dream-dynamic-steady-state-speed-steps} on a
