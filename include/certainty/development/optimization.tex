...
develop a thermal-cycling-aware technique for scheduling of periodic
applications. Note that, by exploring the design space looking for
configurations that reduce the system's wear, we implicitly address condition
uncertainty, which is introduced in \sref{aging-variation}. However, this
approach to condition uncertainty is suboptimal, which is discussed further and
addressed in \sref{chaos-reliability-analysis}.
