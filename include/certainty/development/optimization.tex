...
\sref{dynamic-steady-solution}, we develop a thermal-cycling-aware technique for
...
condition uncertainty, which is introduced in \sref{condition-variation}.
However, this approach to condition uncertainty is suboptimal, which is
discussed further and addressed in \sref{chaos-reliability-analysis}.
...
respectively; see \fref{dream-motivation-graph} for an example. Each pair of a
task and a processing element is characterized by an execution time and a power
consumption; these are the characteristics that the task exhibits when it is
assigned to the processing element. Furthermore, the time to failure of each
processing element is modeled using the reliability model presented in
\sref{thermal-cycling-fatigue}.
...
\begin{align} \elab{thermal-cycling-constraints}
  ...
  \end{split}
\end{align}
In the above formulae, \schedule is a schedule, which specifies the starting
times of the tasks as well as their mapping onto the processing elements;
$\mean_i$ is the \ac{MTTF} of processing element $i$, which is computed
according to \eref{thermal-cycling-mean-time}; \period stands for the end-to-end
delay of the application; and
\[
...
the execution time of each task on each processing element is assumed to be
...
the linear approximation discussed in \sref{power-temperature}. The maximum
temperature $\q_\maximum$ in \eref{thermal-cycling-constraints} is set to
\celsius{100}. In \eref{thermal-cycling-mean-cycles}, the Coffin--Manson
