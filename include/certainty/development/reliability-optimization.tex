...
In the remainder of the section, we first consider a motivational example given
in \sref{thermal-motivation-cycling}. In \sref{thermal-cycling-problem}, we
...
consumption. It can be seen that PE1 is experiencing three thermal cycles as
illustrated by the blue curve, that is, there are three intervals where
...
right-hand side. According to the reliability model given in
\sref{thermal-cycling-fatigue}, these two changes improve the system's lifetime
by around 45\% and 55\%, respectively, relative to the initial schedule given on
the left-hand side of \fref{dream-motivation-cycling}.
...
Static scheduling is considered, which is a task that is typically done offline.
...
The optimization is undertaken via a genetic algorithm \cite{schmitz2004}. In
this paradigm inspired by biology, a population of chromosomes, which represent
candidate solutions, is evolved through a number of generations in order to
