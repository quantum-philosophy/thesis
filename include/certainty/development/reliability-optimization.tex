...
design space in order to find configurations that reduce the system's wear, we
...
suboptimal, which it to be further discussed and addressed in
\sref{chaos-reliability-analysis}.

In the remainder of the section, we first consider a motivational example,
\sref{thermal-cycling-motivation}. In \sref{thermal-cycling-problem}, we
...
\inputfigure{dream-task-graph}
\inputfigure{dream-cycling-motivation}
...
elements denoted by PE1 and PE2. The task graph of the application is given in
\fref{dream-task-graph} along with the execution times of the tasks for both
processing elements. The period of the application is 60~ms. The first
alternative schedule and the resulting dynamic steady-state temperature profile
are shown on the left-hand side of \fref{dream-cycling-motivation} where the
height of a task represents its power consumption. It can be seen that PE1 is
...
seen in the middle of \fref{dream-cycling-motivation}. If we also swap T2 and
T4, the number of cycles of PE1 drops to one, which is depicted on the
right-hand side. According to the reliability model described in
...
respectively; see \fref{dream-task-graph} for an example. The period of the
application is denoted by \period. Furthermore, each pair of a task and a
...
The optimization is carried out via a genetic algorithm \cite{schmitz2004}. In
...
\eref{thermal-cycling-mean-time}, and the fitness function shown in
\eref{thermal-cycling-objective} is calculated.
...
\sref{power-temperature-interplay}. The maximum temperature $\q_\maximum$ in
...
\inputtable{dream-cycling-elements}
...
reported in \tref{dream-cycling-elements}, which also shows the average time
...
\tref{dream-cycling-elements}, the performed optimization does not impact much
the energy efficiency of the system.

\inputtable{dream-cycling-tasks}
...
\tref{dream-cycling-tasks}. The observations are similar to those made with
respect to \tref{dream-cycling-elements}.
...
\inputtable{dream-cycling-methods}
...
\tref{dream-cycling-tasks} with the results produced by the two alternative
...
summarized in \tref{dream-cycling-methods} where \ac{MTTF}\textsubscript{a} and
\ac{MTTF}\textsubscript{b} stand for the two alternative methods, respectively.
It can be seen that, for example, the lifetime of the platform running 160 tasks
can be extended by a factor of 18 using our technique whereas the best solutions
found by the other two techniques within the same time frame are only around
2--5 times better than the initial solution. The reason for the poor performance
of iterative transient analysis with HotSpot is the excessively long execution
time of the calculation of dynamic steady-state temperature profiles, which
means that this method allows for a very shallow exploration of the design
space. In the case of static steady-state analysis, the reason is different: the
method is fast but also very inaccurate as it is discussed and illustrated in
\sref{dynamic-steady-state-prior}. The inaccuracy drives the optimization toward
solutions that turn out to be of low quality.

\inputfigure{dream-cycling-pareto}
...
\fref{dream-cycling-pareto}. It can be observed that the variation of the energy
change is less than 2\%. This means that the solutions optimized for the
