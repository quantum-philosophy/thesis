In this section, we assess the proposed framework for the analysis of process
variation. All the experiments are conducted on a \up{GNU}/Linux machine
equipped with Intel Core i7 2.66~\up{GH}z and 8~\up{GB} of \up{RAM}. All the
...
of applications generated randomly by \up{TGFF} \cite{dick1998}. The model of
...
45-nm \up{PTM} \up{HP} model \cite{ptm}. The temperature calculations are done
using the approach described in \sref{transient-state-solution}; the
...
covariance function in \eref{inference-covariance}, the weight parameter $w$ and
...
the model-order-reduction procedure given in \eref{karhunen-loeve} and utilized
...
\nz of \vz in \eref{inference-reduction} is found to be 27--28. The parameters
...
\sref{bayesian-statistics}, and the second one populates $G$ with $\no = 5
\times 10^3$ elements. For the optimization in \sref{inference-optimization}, we
