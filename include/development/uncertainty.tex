...
Bayesian inference is utilized in \cite{zhang2010} to identify the optimal set
...
Let us consider an important application of the proposed technique: the
...
the left side of \fref{wafer-qoi}. The gradient from navy to dark red represents
...
will be described in detail in \sref{experimental-results}.}
...
Our technique works differently. In order to characterize the effective channel
...
further on this choice in \sref{experimental-results}). We can then apply a
...
wafer are shown on the right side of \fref{wafer-qoi}, and the locations of the
selected dies are depicted in \fref{wafer-measured}. It can be seen that the two
maps in \fref{wafer-qoi} closely match each other implying that our approach is
...
An illustration of this rule is given in \fref{wafer-defective} where the lower
...
In addition, we can introduce a trade-off action: (c) expose the die to a
...
process at hand \cite{chandrakasan2001, cheng2011}; we set $\eta$ to 0.7 and
