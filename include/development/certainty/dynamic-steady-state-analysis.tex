...
the availability of the actual \up{DSSTP}, which we shall illustrate and discuss
further in \sref{thermal-cycling-optimization}.
...
\fref{static-steady-state-error}. It can be seen that, for example, for a
processor area of 10~mm\textsuperscript{2} and an application period of 100~ms,
the \up{NRMSE} of this approximation technique is close to 40\%.
...
500 time steps. The obtained results are shown in \fref{scaling-cores}. It can
be observed that the proposed technique provides a significant performance
