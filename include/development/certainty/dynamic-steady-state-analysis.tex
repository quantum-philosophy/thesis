...
\sref{transient-analysis-present} and the one implemented in HotSpot, and the
corresponding iterative calculation is undertaken until the \up{NRMSE} relative
to the \up{DSSTP} obtained with the proposed method (which is exact) is less
than 1\%.
