...
transient analysis, which is described in \sref{dynamic-steady-state-prior}, and
the one based on the fast Fourier transform, which is mentioned in
\sref{dynamic-steady-state-present}, since they are comparable with ours in
terms of accuracy. In the case of transient analysis, we use the fast analytical
solution described in \sref{transient-analysis-present}, and the corresponding
iterative calculation is undertaken until the \up{NRMSE} relative to the
\up{DSSTP} obtained with the proposed method (which is exact) is less than 1\%.
...
4~mm\textsuperscript{2}. The comparison is depicted in \fref{scaling-steps} on a
