...
shall illustrate in \sref{dynamic-steady-state-result}. Moreover, this
...
dynamic steady-state analysis presented in \sref{dynamic-steady-state-present}
and compare it with the state-of-the-art approaches discussed in
\sref{dynamic-steady-state-prior}. All the experiments are performed on a
\up{GNU}/Linux machine equipped with Intel Core i7 3.4~\up{GH}z and 8~\up{GB} of
\up{RAM}. We also include in the comparison two additional techniques described
in the appendix, namely the TTA with the analytical solution
(\sref{transient-analysis}) and the fast Fourier transform (FFT)
(\sref{straight-forward}). In the cases of the TTA, the simulation over
...
(\sref{power-temperature-interdependence}), execution times remain unchanged; if
considered according to the iterative model
(\sref{power-temperature-interdependence}), execution times increase
