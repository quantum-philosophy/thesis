...
presented in \sref{dynamic-steady-state-analysis} for mitigating the fatigue due
...
We begin with a motivational example. Consider a periodic application with six
tasks denoted by T1--T6 and a heterogeneous platform with two processors denoted
by P1 and P2. The task graph of the application is given in
\fref{thermal-cycling-task-graph} along with the execution times of the tasks
with respect to both processors. The period of the application is 60~ms. The
...
thermal cycles; a thermal cycle is roughly defined as a time interval wherein
...
seen in the middle of \fref{thermal-cycling-motivation}. If we also swap T2 and
T4, the number of cycles of P1 drops to one, which is depicted on the right-hand
side of \fref{thermal-cycling-motivation}. According to the reliability model
that we shall describe shortly, these two changes improve the lifetime of the
...
models that we consider are presented in \sref{system-model} and
\sref{reliability-model}, respectively. In \sref{thermal-cycling-optimization},
...
reported in \sref{thermal-cycling-results}.
...
as discussed and illustrated in \sref{dynamic-steady-state-prior}. The
...
\up{MTTF} have an energy consumption that is almost identical to the one of the
solutions optimized for energy. At the same time, the difference along the
\up{MTTF} dimension is huge. This means that ignoring the reliability aspect the
designer may end up with a significantly decreased \up{MTTF} without any
