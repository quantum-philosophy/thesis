...
list scheduler whose inputs are a set of priorities of the tasks and a mapping
of the tasks onto the processors.
...
\eref{reliability-mean-cycles} is not important for our discussion since we are
concerned with relative improvements as follows.

In each of the experiments, we compare the optimized solution with the initial
one based on the temperature-aware heuristic proposed in \cite{xie2006}. This
heuristic relies on spatial temperature distributions and tries to minimize the
maximum temperature while satisfying real-time constraints. The deadline
$\period$ is set to the duration of the initial schedule extended by 5\%.
