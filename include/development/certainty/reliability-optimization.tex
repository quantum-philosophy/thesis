...
cycling, which is one of the most common failure mechanism \cite{jedec2010}. To
this end, we develop a thermal-cyclic-aware technique for mapping and scheduling
of periodic applications.

\subsection{Motivational Example}

\inputfigure{reliability-task-graph}
\inputfigure{reliability-thermal-cycling}
Consider an application with six tasks denoted T1--T6 and a heterogeneous
platform with two processors denoted P1 and P2. The task graph of the
application is given in \fref{reliability-task-graph} along with the execution
times for both processors. The period of the application is 60~ms. The first
alternative mapping and schedule and the corresponding \up{DSSTP} are shown on
the left-hand side of \fref{reliability-thermal-cycling} where the height of a
task represents its power consumption. It can be seen that P1 is experiencing
three thermal cycles. If we change the mapping and move T6 to P2, the number of
cycles decreases to two, which can be seen in the middle of
\fref{reliability-thermal-cycling}. Finally, if we alter the schedule as well
and swap T2 and T4, the number of cycles of P1 drops to one, which is depicted
on the right-hand side of \fref{reliability-thermal-cycling}. According to the
reliability model that we describe in what follows, these two changes improve
the lifetime of the system by around 45\% and 55\%, respectively, relative to
the initial scheduling and mapping.
