...
thermal conditions change, which is not prominent in \eref{weibull-distribution}
...
= \expectation{T_i}$ the corresponding \up{MTTF}. In the case of
temperature-induced failures, we have that $\shape_i = \shape$ for $i =
...
\range{1}{\ns}$. This part depends on the particular failure mechanism
considered. In the rest of this subsection, we shall tailor the model to the
...
temperature curve, we utilize the so-called rainflow counting method described
in \cite{xiang2010}. As a result, a set of \ns cycles is obtained. Each detected
cycle is characterized by a number of properties including its duration, which
is the desired $\dt_i$. Regarding the corresponding $\mean_i$, it can be
expressed as follows:
\[
  \mean_i = \nc_i \dt_i
\]
where $\nc_i$ stands for the mean number of such cycles to failure. This number
is estimated by using a modified version of the Coffin--Manson equation with the
...
At this point, the reliability model of the component under consideration has
been fully specified. The reliability function is the one in
