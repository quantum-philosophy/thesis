...
The reliability model utilized is the one presented in \cite{huang2009,
xiang2010}. Our focus is restricted to the part of the model that is concerned
with the thermal-cycling fatigue, which is prominently connected to temperature
oscillations.

The model is based on the assumption that the probability distribution
\cite{durrett2010} of the time to failure $T$ is a Weibull distribution, which
is written as
\[
  T \sim \mathrm{Weibull}(\eta, \beta)
\]
where $\eta$ and $\beta$ are the scaling and shape parameters of the
distribution, respectively. The corresponding expectation \cite{durrett2010} is
as follows:
\begin{equation} \elab{weibull-expectation}
  \expectation{T} = \eta \: \Gamma\left(1 + \frac{1}{\beta}\right)
\end{equation}
where $\Gamma$ is the gamma function. The above expected value is the mean time
to failure (\up{MTTF}), which we shall denote by $\theta$ in what follows.

The shape parameter $\beta$ is independent of temperature \cite{chang2006},
which, however, is not the case with the scaling parameter $\eta$. Therefore,
the distribution varies with the operating temperature. In order to take this
into account, we split the period of the application $\tau$ into \nc time
intervals $\{ \dt_i: i = \range{1}{\nc} \}$ so that the thermal properties that
are relevant to the reliability model do not change within each time interval.
Then, we have
\begin{equation} \elab{mean-lifetime-one}
  \eta_i = \frac{\theta_i}{\Gamma\left(1 + \frac{1}{\beta}\right)}
\end{equation}
for $i = \range{1}{\nc}$ where $\theta_i$ is the \up{MTTF} of interval $i$ as if
it was the only interval. The values of $\{ \theta_i: i = \range{1}{\nc} \}$
depend on the particular failure mechanism considered, and we shall tailor them
to thermal cycling shortly.

As it is shown in \cite{xiang2010}, the reliability function, that is, the
function that gives the probability of survival until an arbitrary time moment,
can be approximated using the following formula:
\[
  R(t) = \exp\left\{-\left(\frac{t}{\tau} \sum_{i=1}^\nc \frac{\dt_i}{\eta_i}\right)^\beta\right\}
\]
The formula keeps the form of the Weibull distribution with the scaling
parameter equal to:
\begin{equation} \elab{mean-lifetime-all}
  \eta = \frac{\tau}{\sum_{i=0}^{N_m - 1} \frac{\Delta t_i}{\eta_i}}
\end{equation}
The MTTF with respect to the whole application period can be obtained by
combining \eref{weibull-expectation}, \eref{mean-lifetime-one}, and \eref{mean-lifetime-all}.
...
\eref{mean-lifetime-one}. We focus on the thermal cycling fatigue
...
constant \eref{mean-lifetime-one}, is exactly a thermal cycle.
...
\end{equation}
Taking equations \eref{weibull-expectation}, \eref{mean-lifetime-one},
\eref{mean-lifetime-all}, and \eref{mttf-cycle} together, we obtain the following
