...
\cite{kreith2000}. The core idea is to construct and study an equivalent thermal
\up{RC} circuit of the system under consideration \cite{skadron2003}. The
circuit is a collection of thermal nodes that are characterized by thermal
capacitances and connected with each other via thermal resistances. It is to
capture the relevant physical structure and thermal properties of the system
and, thereby, to model the system's thermal dynamics.
...
package comprises the passive components of the system, that is, those that do
not consume power, as well as any other cooling equipment.

The processing elements can be identified at different levels of granularity
such as \up{SoC}s, \up{CPU}s, and \up{ALU}s. Similarly, an equivalent thermal
\up{RC} circuit of the system can be constructed at different levels of
granularity, which refers primarily to the number and placement of thermal
nodes. It should be noted that the chosen granularity impacts the accuracy of
the subsequent analysis.

Suppose now that an equivalent thermal \up{RC} circuit of the considered system
has been constructed and comprises $\nn$ thermal nodes. Regardless of the
circuit's structure, the thermal dynamics of the circuit are governed by the
...
and $\v{G}$ in \eref{temperature-model-original} depend on the particular model.
Thermal nodes that belong to the package are called inactive, in the sense that
their power dissipation is assumed to be zero.
