...
recall \sref{chaos-surrogate-construction}. When beta distributions are
concerned, the natural choice is the Gauss--Jacobi quadrature; see
\sref{sparse-integration}. Then, using \eref{quadrature-summation}, the
coefficient are computed as shown in \eref{chaos-coefficient}. It is important
to note that \lq should be chosen in such a way that the rule is exact for
polynomials of a total order up to $2 \lc$, that is, twice the level of \ac{PC}
expansions, which can be seen in \eref{chaos-projection} \cite{eldred2008}.
