...
\slab{chaos-problem-formulation}
...
F_i \}_{i = 1}^\nu$ and correlation matrix $\correlation{\vu}$. This information
is already not sufficient to reconstruct the joint distribution in general.
However, the joint distribution can be approximated well by accompanying the
available marginal distributions by a Gaussian copula---which is uniquely
identified by a correlation matrix---constructed based on the available
correlation matrix via the Nataf transformation; see
\sref{probability-transformation}. It should be understood that, even though the
marginal distributions and the Gaussian copula prescribe a joint distribution to
\vu, this distribution is an approximation rather than as the true one.
...
\eref{chaos-expansion} and $\sparseindex{\nz}{\lq}$ in \eref{chaos-coefficient}
