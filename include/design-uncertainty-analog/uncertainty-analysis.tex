...
the operator, namely, the index set $\sparseindex{\nz}{\lq}$, which we discuss
in what follows.
...
$\sparseindex{\nz}{\lc}$ is given a certain ordering so that one can refer to
its elements using an ordinary index $i = \range{1}{\nc}$. Now, let
