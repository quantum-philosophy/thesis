...
\sref{numerical-integration}. To this end, an adequate quadrature is constructed
...
\eref{chaos-coefficient} is a polynomial of total order at most $2 \lc$. With
...
Gaussian quadratures discussed in \sref{numerical-integration}, a quadrature of
...
$\multiindices{\nz}{\lc}$ in \eref{chaos-expansion} and
$\multiindices{\nz}{\lq}$ in \eref{chaos-coefficient} should be synchronized. In
...
\eref{chaos-transform}, a viable option is to rely on the variance contributions
...
$\multiindices{\nz}{l}$, which is also the number of polynomial terms or,
equivalently, coefficients in \eref{chaos-expansion}. Assume that the
multi-index set $\multiindices{\nz}{l}$ is given a certain ordering so that one
...
\multiindices{\nz}{l} \}$ in \eref{chaos-expansion} can now be trivially
computed as
...
\aref{chaos-expansion} where Algorithm~X stands for the routine that calculates
...
The function given by \eref{chaos-expansion} is nothing more than a polynomial;
hence, it is easy to interpret and easy to evaluate. Consequently, having
constructed such an expansion, various statistics about \g can be estimated with
little effort. Moreover, \eref{chaos-expansion} provides analytical formulae for
the expectation and variance of \g solely based on the coefficients of
\eref{chaos-expansion} as follows:
\begin{equation} \elab{chaos-moments}
  ...
  \end{split}
\end{equation}
respectively, where $\v{0} = (0)_{i = 1}^\nz$, and the squaring should be
understood element-wise. Moreover, such quantities as the \ac{CDF} and \ac{PDF}
can be estimated by sampling \eref{chaos-expansion}; each sample is a trivial
...
of Algorithm~X in \aref{chaos-expansion} is assumed to be automatically reshaped
into a row vector.
\end{remark}
