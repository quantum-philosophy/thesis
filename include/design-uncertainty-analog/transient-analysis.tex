...
in \vz and belongs to $\L{2}(\Omega, \F, \probability)$ when it is viewed as a
...
the computation of $\hat{\vp}_{i \vj}$ via $f$ should be done with respect to
the expansion of $\vq_{i - 1}$.
...
\sref{numerical-integration} as well as in \sref{chaos-surrogate-construction}.
