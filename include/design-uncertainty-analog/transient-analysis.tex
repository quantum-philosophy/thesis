...
\sref{chaos-uncertainty-analysis}. In this context, the quantity of interest \g
introduced in \sref{chaos-uncertainty-analysis} stands roughly for the transient
power \mp and temperature \mq profiles of the system. Our goal is to obtain such
a solution that is capable of undertaking the analysis is the same step-by-step
manner as the one inherent to the deterministic solution described in
...
The solution to transient temperature analysis is based on the one presented in
...
last. Consequently, the operations involved in the recurrence are to be
performed on dependent random vectors with arbitrary distributions, which, in
general, have no closed-form solutions, which we address in the following
subsection.
...
Based on the methodology presented in \sref{chaos-uncertainty-analysis}, we
construct a \ac{PC} surrogate for the power function $f$ so that it can be then
propagated through the recurrence in \eref{chaos-recurrence-original} in order
to obtain an expansion for (temperature-aware) power and an expansion for
temperature. Technically, \g in \sref{chaos-uncertainty-analysis} is $f$ in this
section; however, in the end, we arrive at the desired \mp and \mq.
...
is given by \eref{index-total-order-anisotropic}. It can be seen in
...
isotropic total-order multi-index set in \eref{index-total-order-isotropic}, \nc
is given in \eref{index-total-order-isotropic-length}, which behaves as $\nz^\lc
/ \lc!$ in the limit with respect to \nz. For quadratures based on the full
tensor product in \eref{integration-tensor}, $\log(\nq) \propto \nz$, which
...
and is a part of $\transform$ in \eref{chaos-transform}---and \two~utilizing
efficient construction techniques discussed in \sref{numerical-integration} as
well as in \sref{chaos-uncertainty-analysis}. In the case of, for instance,
