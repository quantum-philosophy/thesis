...
procedure delineated in \sref{chaos-surrogate-construction} is applied to
...
which case each coefficient $\hat{\g}_{\multiindex}$ in \eref{chaos-expansion}
is also such a vector. The projection matrix in \eref{chaos-projection-matrix}
and, consequently, \aref{chaos-expansion} should be interpreted as follows: \vg
is an $\nq \times \np \ns$ matrix whose row $i$ is the temperature or power
profile computed at point $i$ of the quadrature in \eref{chaos-coefficient} and
...
the topic of Stage~4 in \fref{chaos-overview}; see \sref{chaos-post-processing}
