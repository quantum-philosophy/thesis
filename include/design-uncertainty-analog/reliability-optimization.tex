...
Our objective in \eref{chaos-objective} is to minimize the expectation of the
...
Our constraints in \eref{chaos-constraints} concern \one~time, \two~temperature,
...
in \rref{chaos-nested-uncertainty}.
...
\vu. In \aref{chaos-expansion}, Algorithm~G is an intermediate procedure that
...
\sref{chaos-surrogate-construction}. On the other hand, the post-processing
stage, Stage~4, is worth discussing.

\subsection{Post-Processing}

Polynomial expansions are utilized inside an optimization procedure whose
objective is to minimize \eref{chaos-objective}. Let us discuss this procedure.
...
checking the timing constraint, the first in \eref{chaos-constraints}, as it
...
lifetime constraints, the last two in \eref{chaos-constraints}, as well as the
evaluation of the expect energy consumption in \eref{chaos-objective} are
...
analytical formula; see \sref{chaos-post-processing}.

\subsection{Illustrative Application}
\inputsection{reliability-application}

\subsection{Experimental Results}
\inputsection{reliability-result}
