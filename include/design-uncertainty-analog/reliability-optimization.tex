...
The general setup is the same as the one in \sref{reliability-optimization}.
However, the goal of our optimization in this section is to find such a schedule
\schedule that minimizes the energy consumption while satisfying certain
constraints. To elaborate, since energy is a function of power, and power
depends on the uncertain parameters \vu, the energy consumption is a random
variable at the design stage, which we denote by $E: \Omega \to \real$. Our
objective is then
\[
...
\begin{align*}
  ...
  & T(\schedule) = \expectation{\left(T(\schedule)\right)} = \eta(\schedule) \Gamma\left(1 + \frac{1}{\beta}\right);
\end{align*}
...
Our constraints in \eref{chaos-constraints} are \one~time, \two~temperature, and
...
system by $\q_\maximum$, and $\rho_\burn$ is an acceptable probability of
burning the chip. The third constrains the lifetime of the system from below by
$T_\minimum$, and $\rho_\wear$ is an acceptable probability of having a
premature fault due to wear. The last two constraints in
\eref{chaos-constraints} are probabilistic since the quantities under
consideration are random. In the last constraint, we set an upper bound on the
probability of the expected value of $T$, and it is important to realize that
this expectation is a random variable itself due to the nested structure of the
